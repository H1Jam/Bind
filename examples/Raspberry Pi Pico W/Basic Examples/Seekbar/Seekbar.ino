 ...
 *
 * This function is automatically triggered when Seekbar 1's value changes on
 * the screen. It receives an integer parameter, 'val,' representing the new
 * value of Seekbar 1. You can define specific actions based on this value to
 * respond to Seekbar changes. To link this callback function with the Bind
 * object, use the following syntax (in the Setup()):
 * `bind.join(your_seekbar_object, seekbar1_changed);`
 * Make sure to include the '&' symbols as they indicate pointers to the
 * function and object.
 * @note Ensure you call "bind.sync();" in the main loop in order to get the
 * callback and chage events.
 *
 ...
 *
 * This function adds (or refreshes, if already exist) SeekBars on the
 * BindCanvas screen. It synchronizes the SeekBars' properties with their
 * respective BindCanvas to display them correctly. You can customize the
 * SeekBars' position, width, maximum value, initial value, and more.
 * @note Ensure you call "bind.sync();" in the main loop in order to get the
 * callback and chage events.
 */
void addSeekBars() {
  ...
  // Tip: You can use the grid view mode inBindCanvas app to determine the x and
  // y coordinates and replace these numbers with the grid values for precise
  // positioning.
  seekBar1.x = 30;
  seekBar1.y = 100;
  /// Set the width of the SeekBar on the screen (height is relative to width
  /// for this object).
  seekBar1.width = 300;
  ...
  /// Specify the command to either add the object to the BindCanvas(screen) or
 ...
 * receive two parameters: 'w' for screen width and 'h' for screen height in
 * density-independent pixels (dp). These values allow you to maintain
...
void setup() {
  ...
  /// @note please ensure that you include the '&' symbols in the function
