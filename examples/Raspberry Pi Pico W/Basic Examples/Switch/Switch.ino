 ...
 *
 * This function is automatically triggered when Switch 1's value changes on the
 * screen. It receives a boolean parameter, 'state,' representing the new state
 * of Switch 1 (ON or OFF). You can define specific actions based on this state
 * to respond to Switch changes. To link this callback function with the Bind
 ...
 * function and object.
 * @note Ensure you call "bind.sync();" in the main loop to get the callback and
 * change events.
 *
 ...
 *
 * This function adds (or refreshes, if already exist) Switches on the
 * BindCanvas screen. It synchronizes the Switches' properties with their
 * respective BindCanvas to display them correctly. You can customize the
 * Switches' position, size, and initial state.
 * @note Ensure you call "bind.sync();" in the main loop to get the callback and
 * change events.
 */
void addSwitches() {
...
void setup() {
  ...
  // Please ensure that you include the '&' symbols in the function parameters;
  // these are pointers.
